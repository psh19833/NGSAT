...
def screen_stocks(
    ...
    for stock_info, price_history in stocks:
        ...
        if candidate and candidate.score >= thresholds["min_score"]:
            ...
            logger.debug(f"스크리닝 저점수: {stock_info.code} {candidate.score:.1f}점 (기준 {thresholds['min_score']})")
...
def _evaluate_single_stock(
    ...
    if atr_pct < 0.2:
        logger.debug(f"스크리닝 필터: {stock.code} ATR {atr_pct:.2f}% < 0.2%")
        return None
    avg_vol = float(np.mean(volumes[-20:])) if len(volumes) >= 20 else 0
    if avg_vol < 500:
        logger.debug(f"스크리닝 필터: {stock.code} 거래량 {avg_vol:.0f} < 500")
        return None
