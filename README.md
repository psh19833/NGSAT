...
> 한국 주식 자동매매 시스템 — 2026.07.03 기준, **70+개** 개선 항목 반영 완료
...
### Phase 0~3 — 전체 코드 리뷰 개선 (26건, 7/3)
P0~P3 전 단계 완료. 외부 리뷰 검증 포함.

| 단계 | 항목수 | 핵심 내용 |
|:----:|:------:|-----------|
| P0 (즉시) | 2 | get_daily_chart→get_price_history, MarketRegime TypeError 수정 |
| P1 (단기) | 9 | partial청산, signal.signal→add_signal_handler, load_config lru_cache, KST 통일, executor Lock, daily_loss fallback, 분봉 timestamp, KOSPI output2 fallback(이미구현) |
| P2 (중기) | 7 | 중앙 Rate Limiter, 보유포지션 universe 포함, PresetRouter hysteresis, bot 캡슐화, ML threshold config 연동, WS 캐시 우선, 분봉 timestamp |
| P3 (장기) | 5 | 백테스트 슬리피지(이미구현), ML NaN→mean 대체, Telegram offset 영속화, 토큰 캐시 base64, trading_loop() 리팩토링(7개 inner function 분리) |
| D2 (已정의) | 3 | auto_tune dict, README 정합, _last_auc(이미구현) |
| **합계** | **26** | **18개 파일 변경, +105/-41라인** |
...
### Phase F — 오늘의 작업 (15+건, 7/3)

| 구분 | 항목 | 내용 |
|:----:|------|------|
| 🐛 **KOSPI 등락률** | 직접 계산 버그 수정 | `index_prices[-1].close` 기준 계산 → KIS API `bstp_nmix_prdy_ctrt` 직접 사용 |
| 🧹 **ETN/ETF 근원 차단** | API 레벨 필터 | `FID_TRGT_EXLS_CLS_CODE=0000001100` (7:ETF, 8:ETN 제외) |
| 🧹 **ETN/ETF 근원 차단** | 유니버스 레벨 필터 | `_get_stock_type()`으로 initialize/swap 단계에서 제외 |
| 📊 **스크리너 Phase 1** | NEUTRAL 컷 완화 | 70→60, MA bearish -20→-10, MACD ±10→±5 |
| 🧹 **유니버스 보강** | DEFAULT 코드 보충 | 랭킹API 30종목 + DEFAULT 10종목 = 40종목 유지 |
| 🐛 **volume_rank API** | 파라미터 오류 수정 | `FID_INPUT_DATE_1` 제거, `FID_TRGT_CLS_CODE` 9자리, `mksc_shrn_iscd` 필드 |
| 🐛 **Semaphore 누락** | Rate Limiter 데드락 | `acquire()`→`async with`로 context manager 사용 |
| 🐛 **ScreenResult frozen** | candidates 필드 할당 오류 | `dataclasses.replace()`로 새 인스턴스 생성 |
| 🐛 **종목명 표시** | ETN 이름 미표시 | `parse_stock_info()`에 `prdt_abrv_name` fallback 추가 |

### 실거래 안전성 개선 (7/3)

| 항목 | 변경 전 | 변경 후 |
|:----|:-------|:--------|
| 유니버스 ETN 비중 | 70%+ (랭킹 API) | **0%** (API 레벨 차단) |
| 스크리너 통과 | 0~2종목 | **7~10종목** |
| KOSPI 인식 | -5.1% (잘못 계산) | **+3.7%** (API 직접 값) |
| NEUTRAL 컷 | 70점 (BULL보다 높음) | **60점** (BULL과 동일) |
| MACD 변동 | ±10점 | **±5점** |
...
| [신규 개발 방향](docs/36-신규개발방향-기획서.md) | 차기 개발 항목 11건 (트레일링스탑/부분청산/앙상블 등) |
| [P1 세부 실행 계획](docs/37-P1세부실행계획서.md) | P1 4건 단계별 리스크 평가 + 세부 구현 계획 |
| [스크리너 개선](docs/38-스크리너-개선-기획서.md) | Phase A/B/C 3단계 스크리너 변별력 개선 |
| [전체코드리뷰 개선](docs/45-전체코드리뷰-개선실행계획서.md) | 22건 P0~P3 전체 수정 완료 |
| [버그 리포트](docs/45-버그리포트.md) | CRITICAL/HIGH/MEDIUM 18건 정리 |
| [외부리뷰 검증](docs/45-외부리뷰-검증보고서.md) | 타 모델 리뷰 48개 주장 코드 검증 |
| [ETF/ETN 제외](docs/48-ETF-ETN-거래제외-계획서.md) | ETN/ETF API 레벨 차단 + 유니버스 필터 |
| [스크리너 분석](docs/49-스크리너-분석리포트.md) | 0건 원인 4가지 분석 및 Phase 1 수정 |
| [프리셋 자동저장](docs/50-프리셋-자동저장-계획서.md) | ConfigService 기반 프리셋 영속화 |
